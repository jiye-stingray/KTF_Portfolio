...
Unity와 C#으로 개발한 모바일 수집형 RPG로, 10인 덱 편성과 다양한 던전 콘텐츠를 핵심으로 하는 한국 배경 판타지 게임입니다. Google Play와 App Store에 출시하여 실 운영 경험을 쌓았으며, Firebase 분석·광고·IAP·어드레서블 리소스 스트리밍 등 모바일 게임 출시에 필요한 전반적인 기술 스택을 직접 구현했습니다. 단순한 기능 구현을 넘어 서버 연동, 보안, 운영 중 라이브 패치까지 아우르는 상용 게임 개발 사이클 전체를 경험하는 것을 목표로 진행한 프로젝트입니다.
