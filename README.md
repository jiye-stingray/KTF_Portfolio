...
### 6. 캐릭터 State Machine 패턴과 Status 관리 분석

인게임 전투 시스템이 어떻게 동작하는지 분석하고 각 구조의 의미를 파악했다.
여러 성장 컨텐츠의 능력치를 연결하여 적용하였다. 

**분석 내용**
- `Squad.cs` 기반 10슬롯 덱 구조, 슬롯 레벨별 해금 조건 처리
- 유닛별 State Machine으로 Idle → Move → Attack → Death 상태 전환 자동화
- `ObjectUnitManager`로 전투 씬 내 유닛 생성·해제 풀링 연결
- `BattleSetting` DBKey를 통한 밸런스 수치 테이블 분리
- 스킬 발동 조건·쿨타임·대상 선택 로직을 `Battle/Skills/` 하위 독립 모듈화
- 상태이상(`StatusEffect`) 시스템으로 버프·디버프 스택 처리

**배운 점**
- State Machine 패턴이 오토배틀처럼 상태 전환이 잦은 시스템에서 코드 유지보수성을 크게 높인다는 것을 실감했다.
- 슬롯 해금 조건을 DB 테이블로 분리하면 기획 변경 시 코드 수정 없이 데이터만 교체할 수 있어 운영 비용이 줄어든다.
